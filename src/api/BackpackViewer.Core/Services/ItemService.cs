...
namespace BackpackViewer.Core.Services
{
    public class ItemService : IItemService
    {
        ...
        private readonly IMockTf2BackpackLoader _mockTf2BackpackLoader;
        private readonly ILogger<ItemService> _logger;

        public ItemService(
            ITf2BackpackLoader tf2BackpackLoader,
            IMockTf2BackpackLoader mockTf2BackpackLoader,
            ILogger<ItemService> logger)
        {
            _tf2BackpackLoader = tf2BackpackLoader;
            _mockTf2BackpackLoader = mockTf2BackpackLoader;
            _logger = logger;
        }
        ...
        public async Task<(IEnumerable<ItemSummary> Items, int BackpackSlots)> GetItemsAsync(ulong steamId, string apiKey, string? playerItemsFile = null, string? itemSchemaFile = null)
        {
            var backpackResponse = playerItemsFile != null
                ? await _mockTf2BackpackLoader.GetItems(playerItemsFile)
                : await _tf2BackpackLoader.GetItems(apiKey, steamId);

            var itemSchemaResponse = (itemSchemaFile != null
                ? await _mockTf2BackpackLoader.GetSchema(itemSchemaFile)
                : await _tf2BackpackLoader.GetItemSchema(apiKey)).ToArray();
        ...
        private IEnumerable<ItemSummary> GetItemSummaries(
            ...
            Dictionary<uint, SchemaItem> itemSchema,
